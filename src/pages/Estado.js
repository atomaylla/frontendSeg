...
const Estado = () => {
    ...
    const saveProduct = async() => {
        ...
        if (estado.descripcion.trim()) {
            ...
            let _aplicacion = { ...estado };
            if (estado.id_estado) {

              //  const index = findIndexById(estado.id_aplicacion);
        ...
        <div className="grid crud-demo">
            <div className="col-12">
                <div className="card">
                    ...
                    <Dialog visible={productDialog} style={{ width: '450px' }} header="Estado" modal className="p-fluid" footer={productDialogFooter} onHide={hideDialog}>

                        <div className="field">
                            <label htmlFor="name">Nombre</label>
                            <InputText id="name" value={estado.descripcion} onChange={(e) => onInputChange(e, 'descripcion')} required autoFocus className={classNames({ 'p-invalid': submitted && !estado.descripcion })} />
                            {submitted && !estado.descripcion && <small className="p-invalid">Ingresar Nombre</small>}
                        </div>

                        <div className="field">
                            <label className="mb-3">Estado</label>
                            <InputSwitch
                                checked={(estado.estado == 1)?true:false}
                    ...
                    <Dialog visible={deleteProductDialog} style={{ width: '450px' }} header="Confirmación" modal footer={deleteProductDialogFooter} onHide={hideDeleteProductDialog}>
                        <div className="flex align-items-center justify-content-center">
                            ...
                            {estado && <span> Deseas  eliminar el estado  <b>{estado.descripcion}</b>?</span>}
                        </div>
                    </Dialog>
