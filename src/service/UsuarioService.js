...
export class UsuarioService {
    ...
    postUsuarioAplicacion(data){
        ...
        return  axiosPrivate.post('usuarioAplicacion/create', JSON.stringify(data),configToken).then(res => res.data).catch(error => {
        });
    ...
    deleteUsuarioAplicacion(data) {
        const token = JSON.parse(sessionStorage.getItem('token'));
        const configToken = { headers: {"Authorization" : `Bearer ${token}`}};
        return  axiosPrivate.delete('usuarioAplicacion/delete', JSON.stringify(data),configToken).then(res => res.data).catch(error => {
        });
    }
    deleteUsuario(id) {
        const token = JSON.parse(sessionStorage.getItem('token'));
